...
fn serve_400(request: Request, message: &str) -> std::io::Result<()> {
    request
        .respond(Response::from_string(format!("400: {message}")).with_status_code(StatusCode(400)))
}

fn serve_api_stats(model: Arc<Mutex<Model>>, request: Request) -> std::io::Result<()> {
    use serde::Serialize;
...
fn serve_bytes(request: Request, bytes: &[u8], content_type: &str) -> std::io::Result<()> {
...
fn serve_api_search(model: Arc<Mutex<Model>>, mut request: Request) -> std::io::Result<()> {
    let mut buf = Vec::new();
...
fn serve_request(model: Arc<Mutex<Model>>, request: Request) -> std::io::Result<()> {
    println!(
