...
fn serve_static_file(request: Request, file_path: &str, content_type: &str) -> std::io::Result<()> {
...
fn serve_api_search(tf_index: &TermFreqIndex, mut request: Request) -> std::io::Result<()> {
    let mut buf = Vec::new();
...
fn serve_request(tf_index: &TermFreqIndex, request: Request) -> std::io::Result<()> {
    println!(
